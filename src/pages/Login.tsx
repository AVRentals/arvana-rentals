...
const Login: React.FC = () => {
    ...
    try {
      ...
      if (authError) {
        ...
        if (authError.message?.includes('fetch') || authError.message?.includes('network') || authError.message?.includes('Failed')) {
          const demoUser = { id: 'demo-' + Date.now(), email, full_name: email.split('@')[0], is_host: false };
  ...
  // Demo login helper
  const fillDemo = () => {
    setEmail('demo@arvanarentals.com');
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-[#0e0e1e] flex items-center justify-center px-4 py-12">
      <div className="w-full max-w-md">
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-muted-foreground">Sign in to your Arvana Rentals account</p>
        </div>
        ...
        <div className="bg-white dark:bg-charcoal-900 rounded-3xl shadow-xl border border-border p-8">
          ...
          <div className="space-y-3 mb-6">
            <button
              ...
              className="w-full flex items-center justify-center gap-3 border-2 border-border rounded-xl py-3 px-4 font-semibold text-sm hover:bg-muted transition-colors"
            >
            ...
            <button className="w-full flex items-center justify-center gap-3 border-2 border-border rounded-xl py-3 px-4 font-semibold text-sm hover:bg-muted transition-colors">
              <svg className="w-5 h-5" viewBox="0 0 24 24" fill="currentColor">
                <path d="M18.71 19.5c-.83 1.24-1.71 2.45-3.05 2.47-1.34.03-1.77-.79-3.29-.79-1.53 0-2 .77-3.27.82-1.31.05-2.3-1.32-3.14-2.53C4.25 17 2.94 12.45 4.7 9.39c.87-1.52 2.43-2.48 4.12-2.51 1.28-.02 2.5.87 3.29.87.78 0 2.26-1.07 3.8-.91.65.03 2.47.26 3.64 1.98l-.09.06c-.22.15-2.18 1.27-2.15 3.79.02 3.02 2.65 4.03 2.68 4.04l-.08.24zM13 3.5c.73-.83 1.94-1.46 2.94-1.5.13 1.17-.34 2.35-1.04 3.19-.69.85-1.83 1.51-2.95 1.42-.15-1.15.41-2.35 1.05-3.11z"/>
              </svg>
              Continue with Apple
            </button>
          </div>
          ...
          <form onSubmit={handleLogin} className="space-y-4">
            ...
            <div>
              <div className="flex items-center justify-between">
                ...
                <button type="button" className="text-xs text-gold-600 dark:text-gold-400 hover:underline font-semibold">
                  Forgot password?
                </button>
              </div>
              <div className="relative mt-1.5">
          ...
          <button
            onClick={fillDemo}
            className="w-full mt-3 text-xs text-muted-foreground hover:text-foreground underline text-center transition-colors"
          >
        ...
        <p className="text-center mt-6 text-sm text-muted-foreground">
          ...
          <Link to="/signup" className="text-gold-600 dark:text-gold-400 font-bold hover:underline">
            Sign up free
          </Link>
        </p>
      </div>
